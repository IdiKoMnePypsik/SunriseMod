...
public class CorePacket {
    ...
    public CorePacket(int pPacketId, DataType[] pDataTypes, Object[] pData, Side pSide) {
    ...
    public static CorePacket read(FriendlyByteBuf pBuf) {
        ...
        return new CorePacket(id, dataTypes, data, side);
    }
    ...
    public static void handle(CorePacket pMessage, Supplier<NetworkEvent.Context> pContext) {
        ...
        context.enqueueWork(() -> {
            if ((pMessage.side == Side.SERVER && context.getDirection().getReceptionSide() == LogicalSide.SERVER) ||
                    (pMessage.side == Side.CLIENT && context.getDirection().getReceptionSide() == LogicalSide.CLIENT)) {
